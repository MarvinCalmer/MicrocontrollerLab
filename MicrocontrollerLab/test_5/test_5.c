...
void TIMER0_IRQHandler(void) {
    if (LPC_TIM0->IR & (1 << 0)) {       // Überprüfen, ob Match0-Interrupt ausgelöst wurde
        ...
        if (increasing) {
            timer_period -= 1000;        // Schrittweise Periodenverringerung für Frequenzanstieg
            if (timer_period <= 62500) { // 800 Hz erreicht, Richtung umkehren
                increasing = 0;
            }
        } else {
            timer_period += 1000;        // Schrittweise Periodenerhöhung für Frequenzabsenkung
