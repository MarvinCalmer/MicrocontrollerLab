...
extern void Timer_Init (uint8_t TimerNum, uint32_t cycle, uint32_t TimerDivid, uint8_t pclksrc , uint8_t ResetMR){
	...
	if (TimerNum==1)
		...
		LPC_SC->PCLKSEL0&=~(3<<4);		//PCLK=CCLK/4 Reset, default
		if (pclksrc==1)
			{LPC_SC->PCLKSEL0|=(1<<4);}
		else if (pclksrc==2)
			{LPC_SC->PCLKSEL0|=(2<<4);}
	...
	if (TimerNum==2)
		...
		LPC_SC->PCLKSEL1&=~(3<<12);		//PCLK=CCLK/4 Reset, default
		if (pclksrc==1)
	...
	if (TimerNum==3)
		...
		LPC_SC->PCLKSEL1&=~(3<<14);		//PCLK=CCLK/4 Reset, default
		if (pclksrc==1)
			{LPC_SC->PCLKSEL1|=(1<<14);}
		else if (pclksrc==2)
			{LPC_SC->PCLKSEL1|=(2<<14);}
		else if (pclksrc==3)
